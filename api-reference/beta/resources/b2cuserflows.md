...
|b2cAuthenticationMethods|String|Used to configure which local accounts are allowed for sign-up/sign-in. The supported values are:<br/><ul><li>`emailWithPassword`</li><li>`userName`</li><li>`phoneWithOneTimePassword`</li></ul>The service allows only certain combinations of `phoneWithOneTimePassword`, `emailWithPassword`, and `userName` to be enabled based on user journey type and other conditions. A v1 or v2 user flow allows one of either the `emailWithPassword` or `userName` methods to authenticate, and it does not allow the `phoneWithOneTimePassword` method. By default, this value will align with the b2cAuthenticationMethod enabled for the tenant. If only one b2cAuthenticationMethod is enabled, that is the default value; otherwise, `emailWithPassword` is the default authentication method.|
...
```json
{
    ...
    "b2cAuthenticationMethods": {"@odata.type": "microsoft.graph.b2cAuthenticationMethods"}
}
```
